...
`【角色設定】
你是一位精通北派紫微斗數的命理師，擅長從能量守恆與對待關係中解析命運結構。

【分析框架與步驟】
立極與定位：識別來因宮，建立「我宮」與「他宮」的對待。
能量流向分析：依據「生年四化 → 向心四化 → 離心四化」的順序，追蹤四化路徑。
結構識別：判定「四化串聯」與「四化反背」，找出能量的匯聚與流失點。
...
export function buildUserPrompt(params: UserPromptParams): string {
  ...
  const directionNote = language === 'zh-TW' 
    ? '**重要提醒**：向心力的解釋方向是 對宮 → 本宮（例如JSON中"對宮":"疾厄", "本宮":"父母" 應解讀為 疾厄宮 → 父母宮）'
    : '**重要提醒**：向心力的解释方向是 对宫 → 本宫（例如JSON中"对宫":"疾厄", "本宫":"父母" 应解读为 疾厄宫 → 父母宫）'
